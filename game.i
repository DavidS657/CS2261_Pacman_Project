...
void drawScore();
# 5 "game.c" 2
# 1 "playerCollisionmap1.h" 1
# 20 "playerCollisionmap1.h"
extern const unsigned short playerCollisionmap1Bitmap[32768];
# 6 "game.c" 2
# 1 "enemyCollisionmap1.h" 1
# 20 "enemyCollisionmap1.h"
extern const unsigned short enemyCollisionmap1Bitmap[32768];
# 7 "game.c" 2
# 1 "playerCollisionmap2.h" 1
# 20 "playerCollisionmap2.h"
extern const unsigned short playerCollisionmap2Bitmap[32768];
# 8 "game.c" 2
# 1 "enemyCollisionmap2.h" 1
# 20 "enemyCollisionmap2.h"
extern const unsigned short enemyCollisionmap2Bitmap[32768];
# 9 "game.c" 2
...
ANISPRITE text[14];
ANISPRITE ghosts[4];
ANISPRITE pellets[80];
ANISPRITE powerPellets[4];
...
void initPlayer() {
    ...
    if(gameProgress % 2 == 0) {
        pacMan.worldRow = 160 / 2 - pacMan.width / 2 + vOff + 18;
    } else {
        pacMan.worldRow = 160 / 2 - pacMan.width / 2 + vOff + 48;
    }

    pacMan.worldCol = 240 / 2 - pacMan.height / 2 + hOff;
    pacMan.aniCounter = 0;
    pacMan.curFrame = 0;
    pacMan.numFrames = 3;
...
void drawScore() {
    for(int i = 0; i < 7; i++) {
        shadowOAM[105 + i].attr0 = (0xFF & score[i].screenRow) | (0<<14);
        shadowOAM[105 + i].attr1 = (0x1FF & score[i].screenCol) | (0<<14);
        if(i == 6) {
            shadowOAM[105 + i].attr2 = ((0)<<12) | ((2)*32+(8));
        } else {
            shadowOAM[105 + i].attr2 = ((0)<<12) | ((2)*32+(scoreDigits[i] + 8));
        }
    }
}
...
void drawLives() {
    shadowOAM[104].attr0 = (0xFF & lives.screenRow) | (0<<14);
    shadowOAM[104].attr1 = (0x1FF & lives.screenCol) | (0<<14);
    if(pacLives == 3) {
        shadowOAM[104].attr2 = ((0)<<12) | ((2)*32+(11));
    }
    else if (pacLives == 2) {
        shadowOAM[104].attr2 = ((0)<<12) | ((2)*32+(10));
    }
    else if (pacLives == 1) {
        shadowOAM[104].attr2 = ((0)<<12) | ((2)*32+(9));
    }
}
...
void drawText() {
    for(int i = 0; i < 14; i++) {
        shadowOAM[i+89].attr0 = (0xFF & text[i].screenRow) | (0<<14);
        shadowOAM[i+89].attr1 = (0x1FF & text[i].screenCol) | (0<<14);
        if(i < 5) {
            shadowOAM[i+89].attr2 = ((0)<<12) | ((1)*32+(8 + i));
        } else {
            shadowOAM[i+89].attr2 = ((0)<<12) | ((0)*32+(3+i));
        }
    }
}
...
void drawPowerPellets() {
    for(int i = 0; i < 4; i++) {
        if (powerPellets[i].hide) {
            shadowOAM[i+85].attr0 |= (2<<8);
        } else {
            shadowOAM[i+85].attr0 = (0xFF & powerPellets[i].screenRow) | (0<<14);
            shadowOAM[i+85].attr1 = (0x1FF & powerPellets[i].screenCol) | (0<<14);
            shadowOAM[i+85].attr2 = ((0)<<12) | ((26)*32+(1));
        }
    }
}
...
void drawPellets() {
    for(int i = 0; i < 80; i++) {
        if (pellets[i].hide) {
            ...
            shadowOAM[i+5].attr0 = (0xFF & pellets[i].screenRow) | (0<<14);
            shadowOAM[i+5].attr1 = (0x1FF & pellets[i].screenCol) | (0<<14);
            shadowOAM[i+5].attr2 = ((0)<<12) | ((26)*32+(0));
    }
    }
}
...
void updateGhost(ANISPRITE* g) {
    ...
    if(g->direction == GHOSTUP) {
        if (g->worldRow > 0 && enemyCollisionmap[((g->worldRow - g->rdel)*(256)+(g->worldCol))] &&
            enemyCollisionmap[((g->worldRow - g->rdel)*(256)+(g->worldCol + g->width - 1))]) {
                g->worldRow -= g->rdel;
            } else {
                ghostRand = rand() % 2;
                if (ghostRand == 0) {
    ...
    if(g->direction == GHOSTDOWN) {
        if (g->worldRow + g->height < 256 && enemyCollisionmap[((g->worldRow - g->rdel + g->height + 1)*(256)+(g->worldCol))] &&
            enemyCollisionmap[((g->worldRow - g->rdel + g->height + 1)*(256)+(g->worldCol + g->width - 1))]) {
                    g->worldRow += g->rdel;
            } else {
                ghostRand = rand() % 2;
                if (ghostRand == 0) {
    ...
    if(g->direction == GHOSTLEFT) {
        if (g->worldCol > 0 && enemyCollisionmap[((g->worldRow)*(256)+(g->worldCol - 1))] &&
        enemyCollisionmap[((g->worldRow + g->height - g->rdel)*(256)+(g->worldCol - 1))]) {
            g->worldCol -= g-> cdel;
        } else {
            ghostRand = rand() % 2;
        if (ghostRand == 0) {
    ...
    if(g->direction == GHOSTRIGHT) {
        if (g->worldCol + g->width < 256 && enemyCollisionmap[((g->worldRow)*(256)+(g->worldCol + g->width))] &&
        enemyCollisionmap[((g->worldRow - g->rdel + g->height)*(256)+(g->worldCol + g->width))]) {
            g->worldCol += g->cdel;
        } else {
                ghostRand = rand() % 2;
        if (ghostRand == 0) {
...
void drawGhosts() {
    for(int i = 0; i < 4; i++) {
        if (ghosts[i].hide) {
            ...
            shadowOAM[i+1].attr0 = (0xFF & ghosts[i].screenRow) | (0<<14);
            shadowOAM[i+1].attr1 = (0x1FF & ghosts[i].screenCol) | (1<<14);

            if (poweredUp == 1 && ghosts[i].gameState != 1 && ghosts[i].gameState != 2) {
                if(powerTimer <= 240 && powerTimer % 60 > 0 && powerTimer % 60 < 30) {
                    shadowOAM[i+1].attr2 = ((0)<<12) | ((24)*32+(ghosts[i].curFrame*2));
                } else {
                    shadowOAM[i+1].attr2 = ((0)<<12) | ((22)*32+(ghosts[i].curFrame*2));
                }
            } else {
                shadowOAM[i+1].attr2 = ((0)<<12) | ((ghosts[i].curFrame * 2 + (4*i + 6))*32+(ghosts[i].aniState * 2));
            }
        }
    }
}
...
void updatePlayer() {
    ...
    if(toggleMovement == 1) {
        if (pacMan.worldRow > 0 && playerCollisionmap[((pacMan.worldRow - pacMan.rdel)*(256)+(pacMan.worldCol))] &&
        playerCollisionmap[((pacMan.worldRow - pacMan.rdel)*(256)+(pacMan.worldCol + pacMan.accWidth - 1))]) {

            pacMan.worldRow -= pacMan.rdel;


            if (vOff > 0 && pacMan.screenRow < 160 / 2 ) {
                vOff--;
            }
        }
    }
    ...
    if(toggleMovement == 2) {
        if (pacMan.worldRow + pacMan.height < 256 && playerCollisionmap[((pacMan.worldRow - pacMan.rdel + pacMan.accHeight + 1)*(256)+(pacMan.worldCol))] &&
        playerCollisionmap[((pacMan.worldRow - pacMan.rdel + pacMan.height + 1)*(256)+(pacMan.worldCol + pacMan.accWidth - 1))]) {
            pacMan.worldRow += pacMan.rdel;

            if (vOff < 256 - 160 && pacMan.screenRow > 160 / 2 ) {
                vOff++;
            }
        }
    }
    if(toggleMovement == 3) {
        if (pacMan.worldCol > 0 && playerCollisionmap[((pacMan.worldRow)*(256)+(pacMan.worldCol - 2))] &&
        playerCollisionmap[((pacMan.worldRow + pacMan.accHeight - pacMan.rdel)*(256)+(pacMan.worldCol - 2))])
             {
                pacMan.worldCol -= pacMan.cdel;
                if (hOff > 0 && pacMan.screenCol < 240 / 2 ) {
                    hOff--;
            }
        }
    }
    if(toggleMovement == 4) {
        if (pacMan.worldCol + pacMan.width < 256 && playerCollisionmap[((pacMan.worldRow)*(256)+(pacMan.worldCol + pacMan.accWidth +2))] &&
        playerCollisionmap[((pacMan.worldRow - pacMan.rdel + pacMan.accHeight)*(256)+(pacMan.worldCol + pacMan.accWidth +2))])
           {
                pacMan.worldCol += pacMan.cdel;
                if (hOff < 256 - 240 && pacMan.screenCol > 240 / 2 ) {
                    hOff++;
                }
            }
    }
...
void drawPlayer() {
    ...
    if (pacMan.hide) {
        ...
        shadowOAM[0].attr0 = (0xFF & pacMan.screenRow) | (0<<14);
        shadowOAM[0].attr1 = (0x1FF & pacMan.screenCol) | (1<<14);
        shadowOAM[0].attr2 = ((0)<<12) | ((pacMan.curFrame * 2)*32+(pacMan.aniState * 2));
    }
}
